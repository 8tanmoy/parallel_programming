...
int main()
{
	printf("#size\t#ev_no\t#step\t#val\t\t\t#error\t\t\t#time(ms)\n");
		//int ex = 5;
	for (int ex = 4; ex < 12; ex++)
	{
		//--construct A--
		int N = 1 << ex;		
		float spf = 0.001;
	...
	//--initialize tenporary variables--
			...
			for (int i = 0; i < M; i++)
			{
				for (int j = 0; j < N; j++)
				{
					sum1[j] += alp[i] * Ab.col(i)[j];	
					sum2[j] += (alp[i] * lam) * b.col(i)[j];	
				}
			}
			q = sum1 - sum2;
	...
	//--Weinstein lower bound convergence check--
			...
			{
				printf("%d\t%d\t%d\t%.15lf\t%.15lf\t", N, k, step-1, lam_old, fabs(lam_abs - lam_old));
				break;
			}
