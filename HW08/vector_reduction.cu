...
float computeOnDevice(float* h_data, int num_elements)
{
	...
	float res;
//	float *temp;
	cudaMalloc(&d_idata, sizeof(float) * num_elements);
	cudaMemset(d_idata, 0.0, sizeof(float) * num_elements);
	cudaMemcpy(d_idata, h_data, sizeof(float) * num_elements, cudaMemcpyHostToDevice);
	int threadsPerBlock = 32;
	int blocksPerGrid = 32;
	cudaMalloc(&d_odata, sizeof(float) * blocksPerGrid);
	cudaMemset(d_odata, 0.0, sizeof(float) * blocksPerGrid);
	reduce<1><<<blocksPerGrid, threadsPerBlock, sizeof(float) * threadsPerBlock>>>(d_idata, d_odata, num_elements);
	
//	temp = (float *)malloc(sizeof(float) * threadsPerBlock);
//	cudaMemcpy(temp, d_odata, sizeof(float) * blocksPerGrid, cudaMemcpyDeviceToHost);
//	for(int j=0; j<blocksPerGrid;j++){printf("%f\n", temp[j]);}
//	free(temp);
	...
	blocksPerGrid = 1;
	cudaFree(d_idata);
	cudaMalloc(&d_idata, sizeof(float));
	cudaMemset(d_idata, 0.0, sizeof(float));
	reduce<1><<<blocksPerGrid, threadsPerBlock, sizeof(float) * threadsPerBlock>>>(d_odata, d_idata, num_elements);
	cudaMemcpy(&res, d_idata, sizeof(float), cudaMemcpyDeviceToHost);
	cudaFree(d_odata);
	cudaFree(d_idata);
	return(res);
}
