...
{   
	...
	Matrix deviceM = AllocateDeviceMatrix(M);
	Matrix deviceN = AllocateDeviceMatrix(N);
	Matrix deviceP = AllocateDeviceMatrix(P);
		
	// copy matrices to device 
	cudaEventRecord(startEvent_exc,0); // staring timing for exclusive
	CopyToDeviceMatrix(deviceM, M);
	CopyToDeviceMatrix(deviceN, N);	

	//launch kernel
	dim3 dimBlock(16, 16);
	dim3 dimGrid((MATRIX_SIZE + dimBlock.x - 1)/dimBlock.x, (MATRIX_SIZE + dimBlock.y - 1)/dimBlock.y);
	MatrixAddKernel<<<dimGrid,dimBlock>>>(deviceM.elements, alpha, deviceN.elements, beta, deviceP.elements);
	...
	CopyFromDeviceMatrix(P, deviceP);
	...
	*exc = elapsedTime_exc;	
	...
	cudaFree(deviceM.elements);
	cudaFree(deviceN.elements);
	cudaFree(deviceP.elements);
}
